...
- [x] 2026-09-22：计划创建。
- [x] P0：路由完整性与角色配置。（2026-09-22：登记 `section_summary`、`course_review`、`course_targeted_repair`，补齐设置页角色映射与路由保存/展示测试。）
- [x] P1：任务级用量可观测性。（2026-09-22：在统一网关记录无提示词的 task/provider/model、结果、耗时与可选 token；新增用户范围聚合接口与设置页摘要，补充迁移、空数据/失败/缺失 token 测试。）
